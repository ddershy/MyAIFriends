...
2. 下载python3.14的XZ compressed source tarball版本，然后将压缩包上传到云服务器。
3. 解压文件：tar -xvf 文件名。

4. 然后手动安装python3.14，先进入已解压的文件夹，然后执行：
...
```
5. 在本地导出python安装过的所有包及其版本号：
   在MyAIFriends目录下执行：`pip freeze > requirements.txt`
6. 然后将`requirements.txt`上传到云服务器上。 scp

7. 然后在云服务上执行：`pip3.11 install -r requirements.txt --user`。

8. 运行完后重新登录一遍服务器，让`/home/acs/.local/bin`自动添加到$PATH中，如果没自动添加，可以手动添加一下。
...
1. 配置不同平台的常量：vue、django、cloud：
  ...
  ```
2. 将前端代码通过`npm run build`打包到后端。

3. 修改数据库中用户（尤其是管理员）的简单密码，防止账号被盗取。

4. 在MyAIFriends目录下上传后端代码：`scp -r backend 服务器名:`