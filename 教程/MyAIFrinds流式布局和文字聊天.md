...
3. 聊天的系统提示词里添加长期记忆;
4. 新建软件包memory和文件graph.py、,`backend/web/views/friend/message/memory/graph.py`,`backend/web/views/friend/message/memory/update.py`；
```py
...
5. 记忆模块需要被调用，调用位置为每次说完话后判断是否超过10条的时候`backend/web/views/friend/message/chat/chat.py`
```py
class MessageChatView(APIView):
    permission_classes = [IsAuthenticated]
    renderer_classes = [SSERenderer] #引入渲染器
    def post(self, request):
        ...
        def event_stream():
            ...
            if Message.objects.filter(friend=friend).count() % 10 == 0:#判断是否有超过10轮，需要记忆
        ...
        return response
```
1. 长期记忆加在系统提示词中，修改 `backend/web/views/friend/message/chat/chat.py`
```py
def add_system_prompt(state,friend):#添加系统提示词
    msgs = state['messages'] #读取之前已有的信息
    system_prompts = SystemPrompt.objects.filter(title="回复").order_by('order_number') #将回复模块按照order_number排序
...
###  8. 知识库-检索增强生成（RAG）
#### 8.1 向量数据库——LanceDB
LanceDB文档：https://docs.lancedb.com/quickstart
1. 在后端装向量数据库相关的包：pip install lancedb langchain-community tantivy。
2. 安装ipython：pip install ipython
3. 新建软件包：`backend/web/documents`
4. 在.gitignore中加入：`backend/web/documents/`。如果不小心索引了文档信息，可以用`git rm --cached -f backend/web/documents/ -r`来删除索引。

#### 8.2 Embedding模型
阿里云text-embedding-v4模型文档：[https://bailian.console.aliyun.com/cn-beijing/?spm=5176.12818093_47.overview_recent.1.1c3716d0UXY98N&tab=api#/api/?type=model&url=2846066]

新建软件包utils,和custom_embddings.py对接阿里云Embedding模型：
```py
...
#### 8.3 插入文档
1. 新建文档`backend/web/documents/data.txt`
2. 测试文档内容:[https://bailian.console.aliyun.com/cn-beijing/?spm=5176.12818093_47.overview_recent.1.1c3716d0UXY98N&tab=doc#/doc/?type=model&url=2579562]
3. 新建文件`backend/web/documents/utils/insert_documents.py`
```py
